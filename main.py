...
start = Tkinter.Button(window, text='Start', width=15, command=lambda: Start(label))
stop = Tkinter.Button(window, text='Stop', width=15, state='disabled', command=Stop)
reset = Tkinter.Button(window, text='Reset', width=15, state='disabled', command=lambda: Reset(label))
...
def clicked2():
    Stop()
    lbl3 = Label(window, text="You are free, now run!")
    lbl3.grid(column=0, row=12)


btn = Button(window, text="Click Me", command=clicked)
btn.grid(column=1, row=0)
btn2 = Button(window, text="Find Hope", command=clicked2)
...
def checked():
	if counter > 500:
		if clicks < 100:
			Stop()
			lbl4 = Label(window, text="Thank you for your sacrifice.", font=("Arial Bold", 50))
		else:
			Reset(label)
			clicks = 0
chk = Checkbutton(window, text="Choose", command=lambda: Start(label) )
